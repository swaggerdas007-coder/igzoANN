...
class TFTNet3(nn.Module):
    """Three-hidden-layer MLP (tanh hidden layers, linear output), for improved
    fit on small-geometry devices:

        h1 = tanh(x  . w1 + b1)
        h2 = tanh(h1 . w2 + b2)
        h3 = tanh(h2 . w3 + b3)
        y  = h3 . wo + bo        (linear output)
    """

    def __init__(self, n_inputs: int = 4, n_hidden1: int = 10, n_hidden2: int = 5,
                 n_hidden3: int = 5, n_outputs: int = 1):
        super().__init__()
        self.hidden1 = nn.Linear(n_inputs, n_hidden1)    # w1, b1
        self.hidden2 = nn.Linear(n_hidden1, n_hidden2)   # w2, b2
        self.hidden3 = nn.Linear(n_hidden2, n_hidden3)   # w3, b3
        self.tanh = nn.Tanh()
        self.output = nn.Linear(n_hidden3, n_outputs)    # wo, bo

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h1 = self.tanh(self.hidden1(x))
        h2 = self.tanh(self.hidden2(h1))
        h3 = self.tanh(self.hidden3(h2))
        y = self.output(h3)
        return y
