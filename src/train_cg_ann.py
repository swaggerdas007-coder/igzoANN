"""Train two-hidden-layer ANNs (10 + 10 tanh neurons) for the a-GIZO TFT
parasitic capacitances C_GD and C_GS, saving everything under trained_Cg_ANN/.

A separate ANN is trained per component (as in Bahubalindruni et al. 2016).
Compared with src/train_cap.py (single 32-neuron hidden layer) this uses a
deeper 4 -> 10 -> 10 -> 1 MLP (src/model.py::TFTNet2), which can bend more
sharply around the threshold step of the measured C-VG curves.

Outputs (per component <cap> in {cgd, cgs}):
    trained_Cg_ANN/<cap>/model_weights.pt      PyTorch state dict
    trained_Cg_ANN/<cap>/weights.json          w1,b1,w2,b2,wo,bo + scaling
    trained_Cg_ANN/<cap>/metrics.json          all test metrics
    trained_Cg_ANN/<cap>/test_predictions.npz
    trained_Cg_ANN/<cap>/plots/scatter_C.png
    trained_Cg_ANN/<cap>/plots/C_vg_curves.png
    trained_Cg_ANN/metrics_summary.json
...
def scatter_plot(cap, c_true, c_pred, m, path):
    ...
    ax.set_title(f"{cap.upper()} 2-layer ANN test set  "
...
def cv_curves(cap, model, y_mean, y_std, path):
    ...
    fig.suptitle(f"{cap.upper()} 2-layer ANN vs measurement (dots=measured, line=model)", y=1.02)
    fig.tight_layout()
...
def train_one(cap_type, verbose=True):
    ...
    model = TFTNet2(n_inputs=4, n_hidden1=N_HIDDEN1, n_hidden2=N_HIDDEN2, n_outputs=1)
    ...
    metrics = {
        "cap_type": cap_type,
        "architecture": {
            "layers": "4 -> 10 (tanh) -> 10 (tanh) -> 1 (linear)",
            "n_inputs": 4, "n_hidden1": N_HIDDEN1, "n_hidden2": N_HIDDEN2, "n_outputs": 1,
    ...
    weights = {
        ...
        "w2": sd["hidden2.weight"].numpy().tolist(),   # [10, 10]
        "b2": sd["hidden2.bias"].numpy().tolist(),     # [10]
        "wo": sd["output.weight"].numpy().tolist(),    # [1, 10]
...
def main():
    ...
    print("=== Summary (2-layer 10+10 ANNs) ===")
